...
const APPLIANCE_META = {
  AC:     { color: "#1d5fa6", bg: "rgba(29,95,166,0.1)",  selBg: "#1d5fa6", icon: <svg viewBox="0 0 24 24"><path d="M4 8a8 8 0 0116 0v8H4V8z"/><path d="M9 16v2a3 3 0 006 0v-2"/></svg> },
  Fan:    { color: "#0a7a72", bg: "rgba(10,122,114,0.1)", selBg: "#0a7a72", icon: <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M12 9c0-3 2.5-5.5 5.5-5.5A5.5 5.5 0 0123 9c0 1.5-1.3 3-3 3H12"/><path d="M12 15c0 3-2.5 5.5-5.5 5.5A5.5 5.5 0 011 15c0-1.5 1.3-3 3-3h8"/></svg> },
  Light:  { color: "#c47c0a", bg: "rgba(196,124,10,0.1)", selBg: "#c47c0a", icon: <svg viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="2"/><path d="M9 18h6"/><path d="M12 2a7 7 0 010 14"/></svg> },
  Fridge: { color: "#6d44b8", bg: "rgba(109,68,184,0.1)", selBg: "#6d44b8", icon: <svg viewBox="0 0 24 24"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="5" y1="10" x2="19" y2="10"/><line x1="10" y1="6" x2="10" y2="8"/></svg> },
  TV:     { color: "#c0354a", bg: "rgba(192,53,74,0.1)",  selBg: "#c0354a", icon: <svg viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="15" rx="2" ry="2"/><polyline points="17 2 12 7 7 2"/></svg> },
};
...
export default function Simulator() {
  ...
  // Update bar chart when result changes
  useEffect(() => {
    if (barChart.current) barChart.current.destroy();
    if (!result || !barRef.current) return;

    const labels = ["Energy Saved", "New Total", "CO₂ Saved", "New CO₂"];
    const values = [
      parseFloat(result.savedUnits),
      parseFloat(result.newTotal),
      parseFloat(result.carbonSaved),
      parseFloat(result.newCarbon),
    ];
    const colors = ["#2ca05a", "#1d5fa6", "#2ca05a", "#c0354a"];
  ...
  return (
    <div className="page">
      <img
        ...
        src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=1200&q=75&fit=crop"
        ...
        <div className="sim-appliance-grid">
          {Object.keys(APPLIANCE_META).map((name) => {
            ...
            return (
              <div
                key={name}
                className={`sim-app-card ${sel ? "selected" : ""}`}
                onClick={() => setSelectedAppliance(name)}
                style={{
                  background: sel ? m.selBg : m.bg,
                  borderColor: sel ? m.color : "var(--border)",
                  color: sel ? "#fff" : m.color,
                  boxShadow: sel ? `0 6px 18px ${m.color}40` : "none",
                }}
              >
                {m.icon}
                <div>{name}</div>
      ...
      {/* Input */}
      <div
        className="premium-card input-card"
        style={{ background: meta.bg, border: `1.5px solid ${meta.color}25` }}
      >
        <p className="section-tag" style={{ color: meta.color }}>
          Simulate: {selectedAppliance}
        </p>
        <p className="info-text">
          Current usage: <strong>{usageHours[selectedAppliance]} hrs/day</strong>
        </p>
        <div className="input-row" style={{ marginTop: 12 }}>
          ...
          <button
            ...
            onClick={simulate}
            style={{ background: meta.color, boxShadow: `0 6px 18px ${meta.color}40` }}
          >
