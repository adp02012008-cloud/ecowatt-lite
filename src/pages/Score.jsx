...
export default function Score() {
  ...
  // Simulated 6-week score history
  const historyScores = [
    Math.max(20, ecoScore - 13),
    Math.max(20, ecoScore - 9),
    Math.max(20, ecoScore - 6),
    Math.max(20, ecoScore - 8),
    Math.max(20, ecoScore - 1),
    ecoScore,
  ];
  ...
  return (
    <div className="page">
      <img
        ...
        src="https://images.unsplash.com/photo-1501854140801-50d01698950b?w=1200&q=75&fit=crop"
        alt="Green landscape"
      />
      ...
      <section className="score-layout">
        ...
        <div className="premium-card score-panel">
          ...
          <div style={{
            ...
            {tiers.find((t) => t.label === status)?.icon &&
        ...
        {/* Tier cards + History chart */}
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {tiers.map((t, i) => (
            <div
              key={i}
              className="score-tier-card"
              style={{
                background: t.bg,
                borderColor: t.label === status ? t.color : t.border,
                boxShadow: t.label === status ? `0 4px 18px ${t.color}25` : "none",
                color: t.color,
              }}
            >
              {t.icon}
              ...
              {t.label === status && (
                <span className="you-tag" style={{ background: t.color }}>YOU</span>
              )}
            </div>
          ))}
          ...
          <div className="premium-card" style={{ padding: 16 }}>
            ...
            <div className="chart-canvas-wrap" style={{ height: 130, marginTop: 8 }}>
              <canvas ref={lineRef} role="img" aria-label="Eco score 6-week history line chart" />
            </div>
          </div>
        </div>
      </section>
